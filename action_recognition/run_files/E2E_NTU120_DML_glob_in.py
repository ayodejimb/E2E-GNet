...
from E2E_models.E2E_RigidNet120_D import RigidNet120
from E2E_models.E2E_NonRigidNet120_D import NonRigidNet120
...
for m in options:
    ...
    for r in range(runs):
        ...
        for dml_run in range(5):
            ...
            if rigid == True:
                model = RigidNet120(mod,num_frames,num_joints, r, dml_run=dml_run, glob_in=True).to(device)
            else:
                model = NonRigidNet120(mod,num_frames,num_joints, r, dml_run=dml_run, glob_in=True).to(device)
